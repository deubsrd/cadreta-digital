...
export type Compra = {
  ...
  created_at: string;
  militares?: Pick<Militar, "id" | "nome" | "identificacao" | "telefone"> | null;
};
...
export async function listMilitares() {
  const { data, error } = await supabase.from("militares").select("*").order("nome");
  if (error) throw error;
  return data as Militar[];
}

export async function upsertMilitar(m: Partial<Militar> & { nome: string; identificacao: string; telefone: string }) {
  if (m.id) {
    const { error } = await supabase.from("militares").update({
      nome: m.nome, identificacao: m.identificacao, telefone: m.telefone, ativo: m.ativo ?? true,
    }).eq("id", m.id);
    if (error) throw error;
  } else {
...
export async function listCompras(opts?: { from?: string; to?: string }) {
  let q = supabase.from("compras").select("*, militares(id,nome,identificacao,telefone)").order("data_compra", { ascending: false });
